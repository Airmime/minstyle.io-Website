...
The `ms-avatar` classe allows you to create a profile image, from an `img` tag or text. Different sizes are available.
...
The presence indicator is used to highlight the user's status. The `ms-status` class allows you to add a small dot under the profile image, through different colors.
...
Finally, to bring out the profile image, the `ms-shadow` class allows you to add a shadow behind the avatar.
